            ...
            if (RetornarId != null)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        foreach (var item in IdsClase)//Contador de creditos
                    ...
                    {
                        MessageBox.Show("Ya ha seleccionado esa clase, porfavos elija otra.");
                    }
                }
            }
        }
            ...
            if (RetornarId != null)
            {
                if (!Valor)
                {
                    try
                    {
                        ...
                        {
                            var aaa = dgvVisualizaconDeDatos.Rows[item.Key].Cells[1].Value.ToString();//Devuelve un null causando excepcion
                            lblClases.Text += dgvVisualizaconDeDatos.Rows[item.Key].Cells[1].Value.ToString() + ", ";//------------------------Excepcion a tratar------------------------
                        }
                        //Salta la excepcion cuando se trata de eliminar una clase previamente seleccionada
                    }
                    catch (ArgumentOutOfRangeException)//------------------------Excepcion a tratar------------------------
                    {
                        MessageBox.Show("Debe seleccionar una clase a eliminar");
                    }
                }
            }
        }
